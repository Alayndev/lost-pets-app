  ...
  render(pets?) {
    ...
    if (pets) {
      ...
      const petsString = hits
        .map((pet) => {
          return `<x-pet-card img=${pet.pictureURL} petId=${pet.objectID} petName="${pet.fullName}" description="${pet.description}" loc="${pet.loc}" type="report" > ${pet.fullName} </x-pet-card>`;
        })
        .join("");
